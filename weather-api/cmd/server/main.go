...
import (
	...
	"weather-api/internal/api/handler"
...
func main() {
	...
	c := client.New()
	wh := handler.NewWeatherHandler(c, log.Logger)
	...
	// Rate limit — Open-Meteo asks for reasonable use
	...
	// ── Weather endpoints
	app.Get("/v1/weather/current", wh.GetCurrent)
	app.Get("/v1/weather/forecast", wh.GetForecast)
	app.Get("/v1/weather/geocode", wh.Geocode)
	...
	app.Get("/health", func(c *fiber.Ctx) error {
		return c.JSON(fiber.Map{"status": "ok", "service": "weather-api", "port": port})
	})

	log.Info().Str("port", port).Msg("weather-api starting")
